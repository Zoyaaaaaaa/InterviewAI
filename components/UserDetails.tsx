...
export default function PlacementPreferences() {
    ...
    const loadExistingData = async () => {
      try {
        ...
        if (response.ok && result.data) {
          setFormData({
            placement_goal: result.data.placement_goal || '',
            preferred_industry: result.data.preferred_industry || '',
            skill_focus: result.data.skill_focus || '',
            interview_preparation: result.data.interview_preparation || '',
          });
          setIsUpdating(true);
        }
      } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showToast(
        isUpdating ? 'Preferences updated successfully!' : 'Preferences saved successfully!',
        'success'
      );
    ...
    <div className="min-h-screen bg-black text-white py-12 px-4 sm:px-6 lg:px-8">
      ...
      >
        <div className="text-center mt-12 mb-12">
          ...
          <p className="text-gray-400 tracking-wider">
            {isUpdating ? 'Update your placement preferences' : 'Customize your placement journey for success'}
          </p>
        </div>
          ...
          {[1, 2, 3, 4].map((i) => (
            <div 
              ...
              className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${
                i <= step ? 'bg-purple-600 text-white' : 'bg-black border border-gray-700 text-gray-500'
              }`}
            >
              {i}
            </div>
        ...
        <div className="mt-12 flex justify-between">
          <motion.button
            ...
            className={`px-6 py-3 rounded-lg transition-all ${
              step === 1 ? 'bg-gray-800 text-gray-500 cursor-not-allowed' : 'bg-purple-700 hover:bg-purple-800 text-white'
            }`}
            whileHover={{ scale: step !== 1 ? 1.05 : 1 }}
            whileTap={{ scale: step !== 1 ? 0.95 : 1 }}
          >
          ...
          <motion.button
            onClick={step < 4 ? handleNextStep : handleSubmit}
            disabled={submitting}
            className="px-6 py-3 bg-purple-700 text-white rounded-lg hover:bg-purple-800 transition-all flex items-center disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          >
            {submitting ? (
              ...
              <>
                {step < 4 ? 'Next' : isUpdating ? 'Update' : 'Complete'}
                {step === 4 && !submitting && <CheckCircle2 className="ml-2" size={20} />}
              </>
            )}
          </motion.button>
        </div>
